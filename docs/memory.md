...
With remembered context disabled, new messages in that conversation do not create memory records and replies do not retrieve saved memory. Existing memories remain until explicitly forgotten or their conversation is deleted. User-notebook saving is a separate visible preference; switching it off retains existing notes. Each source turn keeps its original write permissions, and disabling a branch revokes outstanding updates for it. Re-enabling a branch never retroactively processes messages submitted while that branch was off. A private session avoids durable transcript and generated-notebook storage and uses only explicitly permitted existing context; its work queue is transient. Private mode cannot control provider logs, system crash dumps, or screenshots; its wording must be precise.
