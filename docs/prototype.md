...
- Durable notes jobs run after a completed reply without a separate chat action. Cancelled, interrupted, or malformed derivations leave existing notes intact. Reopening a vault requeues unfinished jobs with bounded automatic retries. Foreground replies preempt background notes work. A failed update offers a contextual retry, including after automatic retries are exhausted; ordinary completed replies have no manual Update notes button. Interrupted replies do not generate notes.
